...
inline P2 screenProject(V3 world, V3 cam, V3 Rv, V3 U, V3 F, double cx, double cy) {
    V3 r = world - cam;
    return project({ dot(r,Rv), dot(r,U), -dot(r,F) }, cx, cy);
}

inline V3 rayFromCursor(s3d::Vec2 p, V3 Rv, V3 U, V3 F, const s3d::Vec2& winF) {
    double sx = p.x - winF.x, sy = -(p.y - winF.y);
    return norm(sx * Rv + sy * U + FOCAL * F);
}

inline std::optional<double> angleFromCursor(s3d::Vec2 p, V3 axis, V3 pivot,
